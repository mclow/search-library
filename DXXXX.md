...
_Note: This is an update of [n34111](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2012/n3411.pdf), presented in Portland._
...
This is one by adding an additional specialization to `std::search`, and some additional functions to create the search objects.
...
A search object is an object that is passed a pattern to search for in it's constructor. Then, the operator () is called to search a sequence for matches.
...
### Calls to be added to the standard library
		...
		template <typename Iterator>
		_unspecified_ make_boyer_moore_searcher ( Iterator first, Iterator last );

		template <typename Iterator>
		_unspecified_ make_boyer_moore_horspool_searcher ( Iterator first, Iterator last );
